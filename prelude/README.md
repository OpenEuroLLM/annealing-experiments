...
```
awk -F, '{if ($1 && $7 > 0) printf("%.6f %s/%s/megatron-lm/%s\n", $7, $1, $2, $3);}' scandinavian1.csv  > scandinavian1.txt awk -F, '{if ($1 && $7) printf("%.6f %s/%s/megatron-lm/%s\n", $7, $1, $2, $3);}' scandinavian1.csv  > scandinavian1.txt
```
